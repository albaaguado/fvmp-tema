/**
 * Block editor component: doo/fap-hero.
 *
 * Inline RichText editing in the canvas.
 * Live course count from FAP CPT.
 */
...
export default function DooFapHeroEdit( { attributes, setAttributes } ) {
	const {
		eyebrow,
		title,
		description,
	...
	const blockProps = useBlockProps( { className: 'doo-fap-hero' } );
	const [ accionesCount, setAccionesCount ] = useState( null );

	useEffect( () => {
		apiFetch( {
			path: '/wp/v2/doo_accion_fap?per_page=1&status=publish',
			parse: false,
		} )
			.then( ( response ) => {
